...
const Sidebar = () => {
    ...
    const [collectList, setCollectList] = useState(getEnWordStore())
    ...
    return (
        <motion.div initial={{width: "120px", position: 'fixed', left: 0, top: 0}}
                    animate={{width: isWider ? 'calc(100vw - 80px)' : '120px'}}
                    style={{zIndex: isWider ? 100 : 0}}
            ...
            <div className="side-bar h-screen text-white relative">
                <div className="flex flex-col absolute left-[60px] top-5 -translate-x-1/2 justify-center items-center">
                    ...
                    <motion.div variants={download_btn_variants}
                                animate={isWider ? 'open' : 'close'}
                                initial={false}>
                        <Button onClick={handleDownloadCSV} className="w-[100px] download--btn mt-6">下载CSV</Button>
                    </motion.div>
                </div>


                <motion.div variants={open_btn_variants}
                ...
                >
                    <div
                        ...
                        className={clsx(['w-[60px]', 'h-[60px]', 'rounded-full', 'bg-second', 'flex', 'justify-center', 'items-center', 'cursor-pointer'])}
                    >
                ...
                <Suspense fallback={<div className="absolute left-[120px] top-5 font-alimama">Loading...</div>}>
                    {isWider && <div
                        className="collect-container absolute top-0 right-0 w-[calc(100%-80px)] h-full flex flex-wrap gap-4 p-10 content-start">
                        <Collect cardList={collectList} onCancelCollect={onCancelCollect}></Collect>
                    </div>}
                </Suspense>
